...
print('EX2-7', next(wi))
print('EX2-8', next(wi))
# print('EX2-9', next(wi)) # StopIteration

# Generator 패턴
# 1. 지능형 리스트, 딕셔너리, 집합 -> 데이터셋이 증가될 경우 메모리 사용량 증가 -> 제네레이터로 완화
# 2. 단위 실행 가능한 코루틴(Coroutine) 구현에 아주 중요
# 3. 딕셔너리, 리스트 한번 호출할 때 마다 하나의 값만 리턴 -> 아주 작은 메모리 양을 필요로 함
...
wg = WordSplitIter('Who says the nights are for sleeping')
wt = iter(wg)
...
# 식만 정의해놓고 next 선언될 때만 동작 -> 메모리 절약 
...
# 그룹화 - groupby
gen9 = itertools.groupby('AAAAABBBCCCCDDDEEE')
